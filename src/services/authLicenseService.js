...
import { constants, createCipheriv, createPrivateKey, generateKeyPairSync, randomBytes, sign } from 'crypto';
import connectToDB from '../config/db.key.connect.js';
import License from '../models/License.js';
import Payment from '../models/Payment.js';
import Plan from '../models/Plan.js';
const AES_KEY = Buffer.from('08bfb5a91d43c4d48600fee85fed9cfe52f945dcca1533a328cd2a1b1ef2942f', 'hex');
...
async function loadOrGeneratePrivateKey() {
  ...
  try {
    ...
    const collection = db.collection('private_keys');
    console.log('Checking for existing private key...');
    ...
    console.error('Error loading or generating private key:', error);
    throw new Error('Failed to load or generate private key');
  }

  if (!privateKey) {
    throw new Error('Private key could not be initialized');
  }
...
async function generateLicense(hardware_id, start_date, expiration_date, transactionId, plan_id, user_id) {
  try {
    ...
    const license = new License({
      ...
      expiration_date: expirationDateObj.toISOString().split('T')[0],
      days_left: daysLeft > 0 ? daysLeft : 0,
      duration_days: planDurationInDays.toString(),
      createdAt: now,
    });
    await license.save();
...
function encryptData(data) {
  ...
  return Buffer.concat([iv, cipher.update(paddedData), cipher.final()]);
}

export { encryptData, ensurePrivateKeyInitialized, generateLicense };
